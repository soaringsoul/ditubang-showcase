    ...
    <div className="relative w-full h-full bg-[var(--bg-primary)] overflow-hidden flex flex-col">
      ...
      <div className="relative px-6 md:px-10 pt-5 pb-2">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-sm font-medium text-[var(--text-tertiary)]">03 / 07</span>
        </div>
      ...
      <div className="relative flex-1 px-6 md:px-10 py-2 overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            {/* 引言 */}
            <div className="p-3.5 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-primary)]">
              <div className="flex items-center gap-2 mb-2">
                <AlertCircle className="w-5 h-5 text-[var(--warning)]" />
                <h3 className="font-bold text-[var(--text-primary)]">真正麻烦的不是某一个功能，而是过程太碎</h3>
              </div>
              <p className="text-xs text-[var(--text-secondary)] mb-2">你手里可能有：</p>
              ...
              <p className="mt-2 text-xs text-[var(--text-primary)] font-medium">
            ...
            <div className="space-y-1.5">
              {painPoints.map((point, index) => (
                <div
                  key={index}
                  className="flex items-center gap-2.5 p-2 rounded-xl bg-red-50/80 border border-red-100 transition-all duration-[var(--transition-fast)] hover:bg-red-50"
                >
                  <div className="w-7 h-7 rounded-lg bg-red-100 flex items-center justify-center shrink-0">
                    <point.icon className="w-3.5 h-3.5 text-red-500" />
                  </div>
                  <span className="text-xs text-[var(--text-secondary)]">{point.text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {/* 解决方案概述 */}
            <div className="p-3.5 rounded-2xl bg-gradient-to-br from-[var(--banana-yellow-pale)] to-[var(--bg-secondary)] border border-[var(--banana-yellow)]/30">
              <div className="flex items-center gap-2 mb-2">
              ...
              <div className="mt-2 grid grid-cols-2 gap-1.5">
                {["查地理数据", "处理表格和位置数据", "做地图和图表", "做空间分析", "覆盖、分单、选址、可达性等业务判断"].map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-secondary)]"
                  >
                    <div className="w-1 h-1 rounded-full bg-[var(--banana-yellow)]" />
            ...
            <div className="p-4 rounded-2xl bg-[var(--black)] text-white">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-sm">说到底，它不是只想帮你"画一张图"</h3>
              </div>
